...
from connector import (
    UserCRUDSQLite,
)  # Importar la clase UserCRUDSQLite desde el módulo connector
...
def test_create_user_with_mocker():  # Definición de un test que utiliza mocker para simular la creación de un usuario
    ...
    mock_cursor.lastrowid = (
        1  # Establecemos la linea del último usuario creado, en este caso el primero
    )

    mock_conn = MagicMock()  # Usamos un mock para el cursor
    mock_conn.__enter__.return_value = mock_conn
    mock_conn.execute.return_value = (
        mock_cursor  # Cuando la comexión ejecute el metodo execute devolvera el cursor.
    )
